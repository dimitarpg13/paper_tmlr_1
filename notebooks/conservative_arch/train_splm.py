...
def main():
    ...
    ap.add_argument(
        ...
        default=False,
        help=(
            "Leave TF32 enabled (CUDA default on Ampere+) instead of forcing "
            "real fp32 matmuls.  Defaults to False (TF32 disabled) because "
            "the SPLM forward uses torch.autograd.grad(create_graph=True) "
            "and that second-order path is sensitive to TF32's 10-bit "
            "mantissa.  Pass --allow-tf32 only to produce a precision-"
            "artifact reference run."
        ),
    )
    args = ap.parse_args()
    ...
    if torch.cuda.is_available():
        if args.allow_tf32:
            ...
            print("[train] TF32 ENABLED (--allow-tf32): producing "
                  "precision-artifact reference; expect numerical drift "
                  "in the autograd.grad(create_graph=True) path")
        else:
            torch.backends.cuda.matmul.allow_tf32 = False
            torch.backends.cudnn.allow_tf32 = False
            print("[train] TF32 disabled (default) for SPLM autograd.grad "

...
def main():
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "real fp32 matmuls.  Defaults to False (TF32 disabled) so the "
            "matched-baseline arm is numerically aligned with the SPLM "
            "cells (which require TF32-off because of "
    ...
    if torch.cuda.is_available():
        if args.allow_tf32:
            ...
            print("[train-matched] TF32 ENABLED (--allow-tf32): producing "
                  "precision-artifact reference run")
        else:
            torch.backends.cuda.matmul.allow_tf32 = False
            torch.backends.cudnn.allow_tf32 = False
            print("[train-matched] TF32 disabled (default) to match the "
                  "SPLM-cell numerical regime (CUDA matmuls in true fp32)")
